...
WITH
  params AS (
    SELECT
      ...
      -- ['2033'] AS new_plu_id,
      ['20', '3012', '3010', '2010', '2011', '3011', '24', '2012']
        AS old_plu_id,
      -- ['2011'] AS old_plu_id,
  ...
  transaction_base AS (
    ...
    WHERE
      ...
      AND t.business_date
        BETWEEN p.pre_launch_begin_date
  ...
  pre_launch_order AS (
    ...
    WHERE
      ...
      AND SAFE_CAST(JSON_VALUE(line_item, '$.q') AS INT64) > 0
  ),
  pre_launch_order_customer AS (
    ...
    GROUP BY accountNumberId, test_store_id
  )
-- SELECT accountNumberId FROM pre_launch_order_customer ORDER BY visit_number DESC
,
-- SELECT * FROM migration2220.golden_chick_item_1c_step1_prior_old_item_buyers_6store_v1
-- =========================================================
...
pre_to_post_launch_population AS (
  ...
  JOIN pre_launch_order_customer pre
    ON
      t.accountNumberId = pre.accountNumberId
      AND t.test_store_id = pre.test_store_id
  CROSS JOIN params p
  WHERE
    t.business_date
    BETWEEN t.launch_date
    AND p.post_launch_end_date
  GROUP BY t.accountNumberId
)  -- SELECT * FROM pre_to_post_launch_population p
  ,
-- =========================================================
...
pre_to_post_new_plu_order AS (
  ...
  WHERE
    ...
    AND SAFE_CAST(JSON_VALUE(line_item, '$.pr') AS NUMERIC) > 0
    AND SAFE_CAST(JSON_VALUE(line_item, '$.q') AS INT64) > 0
  -- AND SAFE_CAST(JSON_VALUE(line_item, '$.p') AS NUMERIC) != 0
),
